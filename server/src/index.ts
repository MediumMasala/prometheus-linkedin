...
import {
  ...
  getSession as getVideoSession,
  getAllSessions as getAllVideoSessions,
  generateTopics,
  selectTopic,
  generateResearch,
  generateScript,
  ...
  generateCaptions,
  assembleVideo,
  // Timing-based scene planning functions (proper flow)
  planScenesFromTiming,           // Step 6: Plan scenes from audio timing
  generateImagePromptsFromSceneBlocks,  // Step 7: Image prompts (scene-aware)
  generateImagesFromPrompts,      // Step 8: Generate images
  generateVideoPromptsFromSceneBlocks,  // Step 9: Video prompts (image + scene aware)
  generateVideosFromPrompts,      // Step 10: Generate videos
  generateCaptionsFromTiming,     // Step 11: Word-level captions
  assembleVideoWithTiming,        // Step 12: Assembly with full metadata
...
// ============== Video Pipeline API (Tal Character Marketing) ==============
// These endpoints are separate from Prometheus analytics to avoid mixing tool calls

// Get all video sessions
app.get('/api/video-pipeline/sessions', (req, res) => {
...
app.post('/api/video-pipeline/sessions', (req, res) => {
  const { theme, character } = req.body;

  if (!theme || !character) {
    return res.status(400).json({ error: 'theme and character are required' });
  }

  const session = createVideoSession(theme, character);
  res.json({ session });
});

// Get session details
app.get('/api/video-pipeline/sessions/:sessionId', (req, res) => {
  const session = getVideoSession(req.params.sessionId);
...
// Generate topics
app.post('/api/video-pipeline/sessions/:sessionId/topics', async (req, res) => {
  try {
    const topics = await generateTopics(req.params.sessionId);
    res.json({ topics });
  } catch (error: any) {
...
// Select topic
app.post('/api/video-pipeline/sessions/:sessionId/select-topic', async (req, res) => {
  ...
  try {
    await selectTopic(req.params.sessionId, topicId);
    res.json({ success: true });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// Generate research (Gemini) - runs in background after topic selection
app.post('/api/video-pipeline/sessions/:sessionId/research', async (req, res) => {
  try {
    const research = await generateResearch(req.params.sessionId);
    res.json({ research });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// Generate script
app.post('/api/video-pipeline/sessions/:sessionId/script', async (req, res) => {
  try {
    const script = await generateScript(req.params.sessionId);
    res.json({ script });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// Generate scenes
app.post('/api/video-pipeline/sessions/:sessionId/scenes', async (req, res) => {
  try {
    const scenes = await generateScenes(req.params.sessionId);
    res.json({ scenes });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// Generate audio (with ElevenLabs timing metadata)
app.post('/api/video-pipeline/sessions/:sessionId/audio', async (req, res) => {
  try {
    const audioUrl = await generateAudio(req.params.sessionId);
    const session = getVideoSession(req.params.sessionId);
    res.json({
      audioUrl,
      audioTimingMetadata: session?.audioTimingMetadata || null,
      hasTimingData: !!session?.audioTimingMetadata?.words?.length,
    });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// ============================================
// NEW: Timing-Based Scene Planning Endpoints
// ============================================

// Plan scenes from ElevenLabs timing (replaces legacy scenes for timing-aware pipeline)
app.post('/api/video-pipeline/sessions/:sessionId/plan-scenes', async (req, res) => {
  try {
    const planningResult = await planScenesFromTiming(req.params.sessionId);
    res.json({
      success: true,
      sentences: planningResult.sentences?.length || 0,
      phraseBeats: planningResult.phrase_beats?.length || 0,
      sceneBlocks: planningResult.scene_blocks?.length || 0,
      audioDuration: planningResult.audio_duration_seconds,
      planningResult,
    });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
...
// Generate image prompts from scene blocks (timing-aware)
app.post('/api/video-pipeline/sessions/:sessionId/scene-image-prompts', async (req, res) => {
  try {
    const result = await generateImagePromptsFromSceneBlocks(req.params.sessionId);
    res.json({
...
// Step 8: Generate images from scene-aware prompts
app.post('/api/video-pipeline/sessions/:sessionId/scene-images', async (req, res) => {
  try {
    const result = await generateImagesFromPrompts(req.params.sessionId);
    res.json({
...
// Step 9: Generate video prompts (image + scene aware)
app.post('/api/video-pipeline/sessions/:sessionId/scene-video-prompts', async (req, res) => {
  try {
    const result = await generateVideoPromptsFromSceneBlocks(req.params.sessionId);
    res.json({
      prompts: result.prompts,
...
// Step 10: Generate videos from prompts
app.post('/api/video-pipeline/sessions/:sessionId/scene-videos', async (req, res) => {
  try {
    const result = await generateVideosFromPrompts(req.params.sessionId);
    res.json({
...
// Step 11: Generate captions from timing metadata (precise word-level)
app.post('/api/video-pipeline/sessions/:sessionId/timing-captions', async (req, res) => {
  try {
    const captions = await generateCaptionsFromTiming(req.params.sessionId);
...
// Assemble video with timing metadata
app.post('/api/video-pipeline/sessions/:sessionId/timing-assembly', async (req, res) => {
  try {
    const result = await assembleVideoWithTiming(req.params.sessionId);
    res.json(result);
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// Get full timing data for a session (for debugging/inspection)
app.get('/api/video-pipeline/sessions/:sessionId/timing-data', (req, res) => {
  const session = getVideoSession(req.params.sessionId);
  if (!session) {
    return res.status(404).json({ error: 'Session not found' });
  }
  ...
  res.json({
    ...
    audioTimingMetadata: session.audioTimingMetadata,
    sentences: session.sentences,
    phraseBeats: session.phraseBeats,
    sceneBlocks: session.sceneBlocks,
    sceneTimeline: session.sceneTimeline,
    scenePlanningResult: session.scenePlanningResult,
    imageAssets: session.imageAssets,
    videoAssets: session.videoAssets,
    captions: session.captions,
  });
});

// ============================================
// END: Timing-Based Scene Planning Endpoints
// ============================================

// Generate image prompts (legacy)
app.post('/api/video-pipeline/sessions/:sessionId/image-prompts', async (req, res) => {
  try {
    const prompts = await generateImagePrompts(req.params.sessionId);
    res.json({ prompts });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// Generate images
app.post('/api/video-pipeline/sessions/:sessionId/images', async (req, res) => {
  try {
    const images = await generateImages(req.params.sessionId);
    res.json({ images });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
app.post('/api/video-pipeline/sessions/:sessionId/video-prompts', async (req, res) => {
  try {
    const prompts = await generateVideoPrompts(req.params.sessionId);
    res.json({ prompts });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// Generate videos
app.post('/api/video-pipeline/sessions/:sessionId/videos', async (req, res) => {
  try {
    const videos = await generateVideos(req.params.sessionId);
    res.json({ videos });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
// Generate captions
app.post('/api/video-pipeline/sessions/:sessionId/captions', async (req, res) => {
  try {
    const captions = await generateCaptions(req.params.sessionId);
    res.json({ captions });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
app.post('/api/video-pipeline/sessions/:sessionId/assembly', async (req, res) => {
  try {
    const videoUrl = await assembleVideo(req.params.sessionId);
    res.json({ videoUrl });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
...
app.get('/api/video-pipeline/sessions/:sessionId/audio.mp3', (req, res) => {
  const audioPath = path.join(__dirname, '..', 'output', 'audio', `${req.params.sessionId}.mp3`);
