...
export function TalLinkedInDashboard() {
  ...
  const checkConnection = async () => {
    try {
      ...
      if (health.linkedInConnected) {
        const accountRes = await authFetch('/api/tal/linkedin/account');
        if (accountRes.ok) {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      const [campaignsRes, analyticsRes] = await Promise.all([
        authFetch('/api/tal/linkedin/campaigns'),
        authFetch(`/api/tal/linkedin/analytics?startDate=${startDate}&endDate=${endDate}`),
      ]);
      ...
      if (campaignsRes.ok && analyticsRes.ok) {
        ...
        setError(errorData.message || 'Failed to fetch Tal LinkedIn data');
      }
    } catch (err) {
      console.error('Failed to load Tal LinkedIn data:', err);
      setError('Failed to load Tal LinkedIn data');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Tal LinkedIn Ads</h3>
              {isConnected && account ? (
      ...
      {isLoading ? (
        <div className="p-8 text-center">
          ...
          <p className="mt-4 text-gray-600">Loading Tal LinkedIn data...</p>
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">Tal Campaign Data</h3>
